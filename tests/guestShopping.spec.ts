...
test('Guest shopping', async ({
	...
	await test.step('Guests should be able to complete payment', async () => {
		...
		await expect
			.soft(orderPage.getOrderNumber())
			.toContainText('Order Number: ORD-');
		await expect.soft(orderPage.getStatus()).toHaveText('Status: Complete');
		await expect
			.soft(orderPage.getOrderDate())
			.toHaveText(`Order Date: ${new Date().toLocaleDateString()}`);
		await expect
			.soft(orderPage.getTotalPaid())
			.toHaveText(`Total Paid: ${expectedTotal}`);
		...
		await expect
			.soft(orderPage.getPostCode())
			.toHaveText(expectedPostCode.toString());
		await expect.soft(orderPage.getCountry()).toHaveText(expectedCountry);
		await expect
			.soft(orderPage.getEmail())
			.toHaveText(`Email: ${expectedEmail}`);
	});
});
